...
plot_opts <- function(i, pl, a){	
	...
	if(!is.na(a[[i]]$panel.bgcolor)) 
	  pl <- pl + theme(plot.background = element_rect(colour=a[[i]]$panel.bgcolor, 
	...
	if(!all(is.na(all_atts(a, 'panel.header')))){
	  ...
	  # All headers must be same size. Default size is 10
		tmp.size <- max(as.numeric(all_atts(a, 'panel.header.size')))*10	
		tmp.lineheight <- as.numeric(all_atts(a, 'panel.header.lineheight'))
		# All line heights must be equal so we use the height 
		...
		ns <- max(unlist(lapply(tmp.headers, 
...
axis_opts <- function(i, pl, a, limsx=NA, limsy=NA, border=TRUE, expx=FALSE, flip = FALSE){
  ...
  pl <- pl + theme(axis.ticks = element_blank())
	...
	# x_continuous must be y_continuous if flip, also apply breaks NULL
	if(flip){
	  
	  ### axis text ###
	  if(a[[i]]$xaxis.ticks.display | a[[i]]$xaxis.text.display) 
	    ystr.breaks <- "" 
	  else
	    ystr.breaks <- ", breaks=NULL" 
	  
	  xstr <- paste(gsub('_x_', '_y_', xstr), ystr.breaks)
